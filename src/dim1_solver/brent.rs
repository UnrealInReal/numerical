...
impl<T: BaseFloat> Dim1Solver<T> for BrentSolver<T> {
    fn solve(&self, func: &impl Dim1Func<T>) -> Option<T> {
        ...
        while (xy_pairs[2].x - xy_pairs[0].x) > self.error_tolerance.double() {
            ...
            if max_error_index != 1 {
                ...
                if next_y == T::ZERO {
                    ...
                    if max_error_index == 0 && next_x > xy_pairs[1].x {
                        ...
                        axis_crossed = xy_pairs[1].y.signum() * xy_pairs[2].y.signum() == -T::ONE;
            ...
            let (next_x, old_interval) =
                if xy_pairs[0].y.signum() * xy_pairs[1].y.signum() == -T::ONE {
                    replaced_index = 2;
                    (
                    ...
                    debug_assert!(xy_pairs[1].y.signum() * xy_pairs[2].y.signum() == -T::ONE);
                    replaced_index = 0;
                    (
            ...
            let new_interval = if replaced_index == 0 {
                if next_y.signum() * xy_pairs[1].y.signum() == T::ONE {
                    xy_pairs[2].x - next_x
                } else {
                    next_x - xy_pairs[1].x
                }
